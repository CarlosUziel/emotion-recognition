...
project.root.dir <- "/media/uziel/DATA/TFG/Verification/"
...
GetConfMatrix <- function(predicted, test.label, threshold = 0.5){
  conf.matrix <- t(as.matrix(table(factor(predicted > threshold, levels=c(F, T)), test.label)))
...
VerificationBalance <- function(data, label.global){
  # balances data according to verification needs. Label 1 vs Label 0
  data.verification <- OneVsAll(data, label.global)
  
  if(sum(data.verification[, 2] == 1) > sum(data.verification[, 2] == 0)){
    # get indices of images with label 1
    indices.1 <- which(data.verification[, 2] == 1)
    # get indices of images with label 0
    indices.0 <- which(data.verification[, 2] == 0)
    # sample label 0 amount of label 1 images
    images.add <- sample(indices.1, sum(data.verification[, 2] == 0))
    # combine label 1 images with label 0 images
    result <- rbind(data.verification[indices.0, ], data.verification[images.add, ])
  } else {
    amount <- round(sum(data.verification[, 2] == 1) / (num.classes - 1))

    images.indices <- as.vector(sapply(c(0:(num.classes - 1))[-(label.global + 1)],
                                       function(label) sample(which(data[, 2] == label),
                                                              amount)))
    
    # get indices of images with label 1
    indices.1 <- which(data.verification[, 2] == 1)
    result <- rbind(data.verification[indices.1, ], data.verification[images.indices, ])
  }
  
  return(result)
}

GetDataSets <- function(data.original, label.global){
  ...
  test <- VerificationBalance(test, label.global)
  # shuffle data
  ...
  eval <- VerificationBalance(eval, label.global)
  # shuffle data
  ...
  train <- VerificationBalance(train, label.global)
  # shuffle data
...
ComputeLabelIteration <- function(data.original, method.iteration, label){
  # compute train and test sets
  model.sets <- GetModelSets(data.original, label)
...
StoreBestModel <- function(eval.error, label.global){
  ...
  prefix <- sprintf("%s_%s_final_%s_label_%s",
                    dataset.name, method.name, dataset.type, label.global)
  mx.model.save(best.model.final, prefix, best.model.method.iteration)
  
}

StoreValidationGraph <- function(train.error, eval.error, label.global){
  # average results
...
StoreEERGraph <- function(FAR, FRR, label.global){
  # average results
  FAR <- colMeans(FAR)
  FRR <- colMeans(FRR)
  ...
  plot.dir <- paste(project.root.dir, "Plots/", sep = "")
  plot.filename <- paste(plot.dir,
                         sprintf("%s_%s_%s_EER_label_%s.png",
                                 dataset.name, method.name,
                                 dataset.type, label.global),
                         sep = "")
  png(filename = plot.filename)
  plot(c(0, 1), c(0, 1), type = "n",
       ...
       main = sprintf("(%s) - Equal Error Rate & Optimum Threshold - Label %s",
                      dataset.name, label.global))
...
StoreROCGraph <- function(graph, label.global){
  # save ROC curve plot
  plot.dir <- paste(project.root.dir, "Plots/", sep = "")
  plot.filename <- paste(plot.dir,
                         sprintf("%s_%s_%s_ROC_label_%s.png",
                                 dataset.name, method.name,
                                 dataset.type, label.global),
                         sep = "")
  png(filename = plot.filename)
  plot(0,
       ...
       main = sprintf("(%s) - Receiver Operating Characteristic (ROC) - Label %s",
                      dataset.name, label.global))
  # plot ROC
  plot(graph, avg = "vertical", col = "blue", lwd = 2.5)
  # plot diagonal line
  lines(x = c(0, 1), y = c(0, 1), lwd = 3)
  legend(0.7, 0.15, sprintf("Label %d", label.global),
         lty = 1, lwd = 2.5, col = "blue")
  dev.off()
}

StoreGlobalROCGraph <- function(ROC.curves){
  # save global ROC curve plot
  plot.dir <- paste(project.root.dir, "Plots/", sep = "")
  plot.filename <- paste(plot.dir,
                         sprintf("%s_%s_%s_ROC_global.png",
  ...
  plot(0,
       xlim = c(0, 1), ylim = c(0, 1), type = "n",
       xlab = "False Positive Rate", ylab = "True Positive Rate",
       main = sprintf("(%s) - Receiver Operating Characteristic (ROC) - Global",
                      dataset.name))
  
  colors <- c("red", "green", "blue", "brown",
              "cyan", "mediumpurple", "darkorange", "deeppink")
  
  # plot FPR and TPR
  for( i in (1:num.classes)){
    plot(ROC.curves[[i]], avg = "vertical", col = colors[i], lwd = 2.5, add = TRUE)
  }
  
  # plot diagonal line
  lines(x = c(0, 1), y = c(0, 1), lwd = 3)
  legend(0.75, 0.5,
         c("Label 0", "Label 1", "Label 2", "Label 3",
...
GetLabelMetrics <- function(data.original, label.global){
  ...
  method.result <- t(sapply(1:method.iterations, function(method.iteration)
    ComputeLabelIteration(data.original, method.iteration, label.global)))
  ...
  StoreBestModel(unlisted.errors$eval.error, label.global)

  # compute and store validation results graph
  StoreValidationGraph(unlisted.errors$train.error,
                       unlisted.errors$eval.error, label.global)
  
  # get EER curves
  unlisted.EER <- UnlistEER(method.result[, 4:5])
  
  # compute and store EER graph
  StoreEERGraph(unlisted.EER$FAR, unlisted.EER$FRR, label.global)

  # get ROC curves
  ROC.curve <- ComputeROCCurve(method.result[, 6], method.result[, 7])
  
  # compute and store ROC graph
  StoreROCGraph(ROC.curve, label.global)
  ...
  # mean and variance of all iterations
  method.metrics <- ReorderVector(colMeans(method.metrics),
                                  colVars(method.metrics))
  
  return(list("metrics" = method.metrics,
              "ROC.curve" = ROC.curve))
}

OneVsAll <- function(data.original, label.global){
  # verification --> one label vs the rest
  data.original[, 2] <- as.integer(data.original[, 2] == label.global)
  return(data.original)
}

GetMethodMetrics <- function(data.original){
  # get metrics for each label
  label.result <- t(sapply(0:(num.classes - 1), function(label.global) GetLabelMetrics(data.original, label.global)))
  
  # store ROC graph containing all labels
  StoreGlobalROCGraph(label.result[, 2])
  
  # METRIC1, VARIANCE2, METRIC2, VARIANCE2, ...
  label.metrics <- matrix(unlist(label.result[, 1]),
                           ncol = length(unlist(label.result[1, 1])),
                           byrow = TRUE)
...
GetAllVerificationMetrics <- function(data.original){
  # compute all metrics
